            ...
            <Section key={sec.id} id={sec.id} icon={sec.icon} title={sec.title} open={openSection === sec.id} onToggle={(id) => setOpenSection((cur) => (cur === id ? null : id))}>
              ...
              {sec.id === 'appearance' && (
                <>
                  ...
                  <select className="form-input" value={draft.wallpaper} disabled={disabled} onChange={(e) => updateDraft('wallpaper', e.target.value)}>
                    <option value="default">Default Dark Blue</option><option value="minimal">Minimal Grid</option><option value="soft">Soft Gradient</option>
                  </select>
                  ...
                  <p className="settings-hint">Font size, density and animation preferences are saved on this device.</p>
                </>
              )}
              ...
              {sec.id === 'notifications' && (
                <>
                  ...
                  <SelectRow label="Notification Preview" value={draft.notification_preview} disabled={disabled} onChange={(v) => updateDraft('notification_preview', v)}
                    options={[{ value: 'show', label: 'Show Message' }, { value: 'hide', label: 'Hide Message' }]} />
                  <SelectRow label="Notification Sound" value="default" disabled options={[{ value: 'default', label: 'Default' }]} />
                  <ToggleRow label="Vibration" checked={draft.vibration} disabled={disabled} onChange={(v) => updateDraft('vibration', v)} />
                  <p className="settings-hint">
                    Sound, preview and vibration preferences are saved on this device. Selectable notification
                    sounds need audio assets that aren't in the project yet - the picker is a placeholder until
                    those are added.
                  </p>
                </>
              )}
