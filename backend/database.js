  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS user_settings (
        ...
        notification_preview TEXT DEFAULT 'show',
    ...
    // Notification-preference columns. These used to live only in
    // localStorage on the client, which meant toggling them never actually
    // changed what the backend sent via FCM — the switches were fake. They
    // now live here so send_message / friend-request push dispatch can
    // check them per-recipient before calling push.sendToUser().
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS message_notifications BOOLEAN DEFAULT TRUE;`);
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS group_notifications BOOLEAN DEFAULT TRUE;`);
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS friend_request_notifications BOOLEAN DEFAULT TRUE;`);
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS notification_preview TEXT DEFAULT 'show';`);
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS vibration BOOLEAN DEFAULT TRUE;`);
    // Appearance columns that generally affect layout only (device-appropriate
    // to sync across a re-login/reinstall on any device, unlike wallpaper's
    // custom gallery image which is device-local binary data).
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS font_size TEXT DEFAULT 'medium';`);
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS chat_density TEXT DEFAULT 'comfortable';`);
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS message_animations BOOLEAN DEFAULT TRUE;`);
    await client.query(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS ui_animations BOOLEAN DEFAULT TRUE;`);
    await client.query(`UPDATE user_settings SET
      ...
      group_notifications = COALESCE(group_notifications, TRUE),
      friend_request_notifications = COALESCE(friend_request_notifications, TRUE),
      notification_preview = COALESCE(notification_preview, 'show'),
      vibration = COALESCE(vibration, TRUE),
      font_size = COALESCE(font_size, 'medium'),
      chat_density = COALESCE(chat_density, 'comfortable'),
      message_animations = COALESCE(message_animations, TRUE),
      ui_animations = COALESCE(ui_animations, TRUE);
    `);
