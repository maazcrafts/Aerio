...
const ensureUserSettings = async (userId) => {
  ...
  const { rows } = await db.query('SELECT theme, accent_color, notification_sounds, wallpaper, privacy_last_seen, privacy_profile_visibility, read_receipts, typing_indicator, online_status_visibility FROM user_settings WHERE user_id = $1', [userId]);
  const row = rows[0] || {};
  // Ensure defaults are provided for any missing fields
  return {
    theme: row.theme ?? DEFAULT_USER_SETTINGS.theme,
    accent_color: row.accent_color ?? DEFAULT_USER_SETTINGS.accent_color,
    notification_sounds: row.notification_sounds ?? DEFAULT_USER_SETTINGS.notification_sounds,
    wallpaper: row.wallpaper ?? DEFAULT_USER_SETTINGS.wallpaper,
    privacy_last_seen: row.privacy_last_seen ?? DEFAULT_USER_SETTINGS.privacy_last_seen,
    privacy_profile_visibility: row.privacy_profile_visibility ?? DEFAULT_USER_SETTINGS.privacy_profile_visibility,
    read_receipts: row.read_receipts ?? DEFAULT_USER_SETTINGS.read_receipts,
    typing_indicator: row.typing_indicator ?? DEFAULT_USER_SETTINGS.typing_indicator,
    online_status_visibility: row.online_status_visibility ?? DEFAULT_USER_SETTINGS.online_status_visibility,
  };
};
...
app.put('/api/settings/user', authenticateToken, async (req, res) => {
  ...
  const validWallpapers = new Set(['default', 'minimal', 'soft']);
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO user_settings (user_id, theme, accent_color, notification_sounds, wallpaper, privacy_last_seen, privacy_profile_visibility, read_receipts, typing_indicator, online_status_visibility)
       ...
       RETURNING theme, accent_color, notification_sounds, wallpaper, privacy_last_seen, privacy_profile_visibility, read_receipts, typing_indicator, online_status_visibility`,
      [userId, theme, accent_color, notification_sounds, wallpaper, privacy_last_seen, privacy_profile_visibility, read_receipts, typing_indicator, online_status_visibility]
    );
              ...
              if (groupId) {
                ...
                await Promise.all(recipients.map(async (uid) => {
                  ...
                  await push.sendToUser(uid, {
                    title: groupName,
                    body: `${senderUsername}: ${previewText}`,
                    data: {
                      type: 'group_message',
